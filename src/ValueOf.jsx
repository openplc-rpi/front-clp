...
function ValueOf({ id, data }: NodeProps<Node<{ text: string }>>) {
  ...
  return (
    ...
    >
      <div><center>Get Value Of</center></div>
      <center>
        <div style={{ marginTop: 5 }}>
        <select value={data.text} id="value_of" name="value_of" style={{ display: 'block', width: 50, fontSize: 10 }} onChange={(evt) => updateNodeData(id, { text: evt.target.value })}>
          {options}
        </select>          
        </div>
      </center>
